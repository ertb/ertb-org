 ...
 * @param {(Db|DbResolver)} options.db Mongo database. Uses req.db if unset.
...
export const MongoRestRouter = <T extends object>(collection:string, schema:JSONSchemaType<T>, options={} as MongoRestRouterOptions): Router => {
  ...
  if (db) {
    // middleware to attach Db to request context
    ...
    router.post('/', async (req:Request, res:Response)=>{
      try {
        ...
        if (Array.isArray(payload)) {
          ...
          if (noPostBulk) {
            throw new ValidationError('should be an object')
          }
  ...
  if (!methods || methods.includes('PUT')) {
    ...
    router.put(idPath, async (req:Request, res:Response)=>{
      try {
        const payload = validate(req.body, {isUpdate:true}) as {_id?:ObjectId}
